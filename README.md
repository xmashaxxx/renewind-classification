# ReneWind — Cost-Sensitive Turbine Failure Predictor
...
Because replacement costs 5× more than repair, **recall on the failure class is the primary metric** — not accuracy, not F1. A model that catches every failure at the cost of some false alarms is preferable to one that is "accurate" but misses failures.
